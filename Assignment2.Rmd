...
ean_profit_by_month <- aggregate(profit ~ release_month, data = movies1, FUN = mean, na.rm = TRUE)
...
```{r}
#WRITE YOUR CODE HERE
movies1_positive$english_dummy_var <- ifelse(movies1_positive$original_language == "en", 1, 0)

movies1_positive <- subset(movies1, profit > 0 & budget > 0)

ols_model<-lm(log(profit)~log(budget)+english_dummy_var+release_month, data = movies1_positive)
