...
```{r}
movies1<-read.table("movies1.tsv", sep="\t", header = TRUE)
...
mean_profits<-mean(movies1$profits)
median_profits<-median(movies1$profits)
max_profit<-max(movies1$profits)
min_profits<-min(movies1$profits)
...
movies1[which.max(movies1$profits), "title"]
movies1[which.min(movies1$profits), "title"]
...
quantile(movies1$profits, probs = c(0.25, 0.5, 0.75, 1))
...
movies1$log_profits<-log(movies1$profits)

mean(movies1$log_profits, na.rm = TRUE)
...
mean(movies1$log_profits, na.rm = FALSE)

ggplot(movies1, aes(x=log_profits))+
  geom_boxplot()+
  labs(title = "Distribution of Profits",
        xlab = "movies",     
        ylab = "log_ptrofits")+
...
ggplot(movies1, aes(x = runtime, y = vote_average))+
  geom_point()+
